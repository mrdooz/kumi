...
float BSplineEval(float t, float p0, float p1, float p2, float p3) {
  float t2 = t*t;
  float t3 = t2*t;
...
void stepBSpline(int pointsPerSegment, const vector<XMFLOAT3> &controlPoints, vector<XMFLOAT3> *out) {
  ...
  for (int i = 0; i < numPts - 1; ++i) {
    ...
    for (int j = 0; j < pointsPerSegment; ++j) {
      ...
      dst->x = BSplineEval(t, p0.x, p1.x, p2.x, p3.x);
      dst->y = BSplineEval(t, p0.y, p1.y, p2.y, p3.y);
      dst->z = BSplineEval(t, p0.z, p1.z, p2.z, p3.z);
      dst++;
    }
  }
}
...
bool SplineTest::init() {
  ...
  _vb = GRAPHICS.create_dynamic_vertex_buffer(FROM_HERE, _particle_data.numParticles * sizeof(ParticleVtx));
...
bool SplineTest::render() {
  ...
  ScopedRt rtCoalesce(w, h, fmt, Graphics::kCreateSrv, "rtCoalesce");
  {
    ...
    _ctx->render_technique(_coalesce, bind(&SplineTest::fill_cbuffer, this, _1), arr, DeferredContext::InstanceData());
    _ctx->unset_render_targets(0, 1);
  }
...

  {
    ...
    _ctx->set_default_render_target();
    _ctx->render_technique(_compose_technique, bind(&SplineTest::fill_cbuffer, this, _1), arr, DeferredContext::InstanceData());
  }
