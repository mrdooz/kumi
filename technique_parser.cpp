...
public:
	...
	TechniqueParser() {
		for (int i = 0; i < ELEMS_IN_ARRAY(g_symbols); ++i)
			_symbols_trie.add_symbol(g_symbols[i].str, strlen(g_symbols[i].str), g_symbols[i].symbol);
	}

	const char *next_symbol(const char *start, const char *end, Symbol *symbol) {
		if (skip_whitespace(&start, end) == end)
			return end;

		int len = 0;
		Symbol s = _symbols_trie.find_symbol(start, end - start, &len);
		// skip comments
	...
	bool expect(Symbol symbol, const char **start, const char *end) {
		Symbol tmp;
...
#pragma warning(suppress: 4996)
	if (!(f = fopen("C:/Users/dooz/Dropbox/material1.txt", "rb")))
#pragma warning(suppress: 4996)
		f = fopen("C:/Users/dooz/Documents/My Dropbox/material1.txt", "rb");
	...
	TechniqueParser m;
	bool res = m.parse_main(buf, buf + size);
