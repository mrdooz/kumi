...
public:
	TechniqueParser();
	~TechniqueParser();
	bool parse(const char *start, const char *end, vector<Technique *> *techniques, vector<Material *> *materials);
private:
	friend struct Scope;
  ...
  void parse_param(const vector<string> &param, Technique *technique, ShaderTemplate *shader);
